...
fn read_json_from_file<P: AsRef<Path>>(path: P) -> Result<Vec<Sprite>, Box<Error>> {
    // Open the file in read-only mode with buffer.
    let file = File::open(path)?;
    let reader = BufReader::new(file);

    // Read the JSON contents of the file as an instance of `User`.
    let u: Vec<Sprite> = serde_json::from_reader(reader)?;
...
impl SimpleState for MyState {
    ...
    fn on_start(&mut self, data: StateData<'_, GameData<'_, '_>>) {
        ...
        let u = read_json_from_file("sprite_loader.json").unwrap();
        println!("{:#?}", u);
    }
...
/*pub fn character_creation_ui(world: &mut World) {
    let plus_button = world.create_entity().with(UiImage::Texture);
}*/
